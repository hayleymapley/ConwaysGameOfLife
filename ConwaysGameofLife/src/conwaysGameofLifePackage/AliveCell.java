...
public class AliveCell extends Cell {
	...
	public void spawnDeadCells(Grid worldGrid) { 
		...
		for(Node c : worldGrid.getCellGroup().getChildren()) {
			...
			//If current check has same position as position checked then return false (not a valid place to spawn
			// checks top left
		...
		if (tl == true) {		//can draw alive cell here
			if(worldGrid.isTestCellEmpty(this.getxPos() - getSize(), this.getyPos() - getSize())) {
				TestCell topLeft = new TestCell(this.getxPos() - getSize(), this.getyPos() - getSize());	//new dead cell at coordinates checked
